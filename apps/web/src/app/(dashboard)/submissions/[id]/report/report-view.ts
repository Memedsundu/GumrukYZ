...
/**
 * Shared server-side assembly of the risk-report view model. Both the Risk Raporu
 * (`/report`) and Aksiyon Listesi (`/actions`) surfaces read from this so the
 * ~280-line build lives in one place. Returns `null` when no report exists yet;
 * throws `notFound()` when the submission is not found for this tenant.
 */
export async function loadReportView(id: string): Promise<ReportViewModel | null> {
